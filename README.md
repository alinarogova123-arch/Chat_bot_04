...
Скрипт содержит набор функций для взаимодействия бота с Strapi.
...
Возращает массив со всеми экземпляров рыбы из модели Fish или конкретный экземляр, если передан аргумент document_id
...
Проверяет существует ли корзина пользователя по ID пользователя мессенджера, возвращает `documentId` корзины, либо `None`,
